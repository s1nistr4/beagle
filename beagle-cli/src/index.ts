...
function renderFile(path:string, rootPath:string) {
  const isPug:RegExp = /\.(?:pug|jade)$/;
  const isIgnored:RegExp = /([\/\\]_)|(^_)/;
  let stat:Stats = fs.lstatSync(path);
...
function getNameFromFileName(filename:string):string {
  let file:string = basename(filename).replace(/\.(?:pug|jade)$/, '');
