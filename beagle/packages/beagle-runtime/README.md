...
```js
var build = require('beagle-runtime/build');
var src = build(['attr']);
...
When testing code compiled for the browser in Node.js, it is necessary to make the runtime available. To do so, one can use `require('beagle-runtime/wrap')`:

```js
var pug = require('beagle-main');
